...
<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
  ...
  //   O use state ajudará a ver qual botáo foi copiado, nesse caso está null porque nenhum botão aindo foi copiado
  // quando um botão for copiado
  const [copiarIndex, setCopiarIndex] = useState(null);
  //  Para copiar os svgs deve-se usar o api navigator.clipboard q permite copiar para area de transferência do usuario
  const copiarComApiClipboard = (codigoSvg, index) => {
    navigator.clipboard.writeText(codigoSvg).then(() => {
      setCopiarIndex(index);
      setTimeout(() => setCopiarIndex(null), 2000);
    });
  };
  // Renderiza os SVGs dinamicamente
  return (
    <ul className="grid 2xl:grid-cols-5 lg:grid-cols-4 lg:gap-x-10 md:grid-cols-3 grid-cols-2 w-full lg:w-auto gap-5 px-5 2xl:gap-10">
      {svgElements.map((element, index) => (
        <li key={index} className="text-center">
          <div className="flex flex-col items-center space-y-5 py-10 px-2 rounded-lg shadow-sm border border-[#272727]">
            <div
              className="text-slate-200"
              dangerouslySetInnerHTML={{ __html: element.svg }}
            />
            <p className=" text-sm text-slate-300">{element.name}</p>
          </div>
          <button
